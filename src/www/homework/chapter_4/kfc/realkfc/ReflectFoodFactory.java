package www.homework.chapter_4.kfc.realkfc;
...
public class ReflectFoodFactory {
    ...
    public static float selectPay(float shouldPayMoney) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        int selcet = get13();
        if (selcet == 1) {
            pay = (Pay)Class.forName("www.homework.chapter_4.kfc.realkfc.AliPay").newInstance();
            HandlerPay handlerPay = new HandlerPay(pay);
            resMoney = handlerPay.countMoney(shouldPayMoney);
        } else if (selcet == 2) {
            pay = (Pay)Class.forName("www.homework.chapter_4.kfc.realkfc.TencentPay").newInstance();
            HandlerPay handlerPay = new HandlerPay(pay);
            resMoney = handlerPay.countMoney(shouldPayMoney);
        } else if (selcet == 3) {
            pay = (Pay)Class.forName("www.homework.chapter_4.kfc.realkfc.IPay").newInstance();
            HandlerPay handlerPay = new HandlerPay(pay);
            resMoney = handlerPay.countMoney(shouldPayMoney);
        }
        return resMoney;
